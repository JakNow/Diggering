...
public class SphereCollider extends CollisionShape {
	...
	public boolean intersection(AABB aabb) {
		...
		if (distance < this.radius) { isIntersecting = true; }
	...
	public boolean intersection(SphereCollider sphereCollider) {
		...
		boolean isIntersecting =
				(this.tempCenter.distance(sphereCollider.tempCenter) < (this.radius + sphereCollider.radius));
	...
	public boolean intersection(CylinderCollider cylinderCollider) {
		...
		float dy = Math.max(cylinderCollider.getTempCenter().y - cylinderCollider.getHeight(),
				Math.min(this.tempCenter.y, cylinderCollider.getTempCenter().y + cylinderCollider.getHeight()));

		float distance = (float) Math.sqrt((this.tempCenter.x - cylinderCollider.getTempCenter().x) *
				(this.tempCenter.x - cylinderCollider.getTempCenter().x) +
				(dy - cylinderCollider.getTempCenter().y) * (dy - cylinderCollider.getTempCenter().y) +
				(this.tempCenter.z - cylinderCollider.getTempCenter().z) *
						(this.tempCenter.z - cylinderCollider.getTempCenter().z));

		if (distance < cylinderCollider.getRadius() + this.radius) { isIntersecting = true; }
