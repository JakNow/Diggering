...
public class CylinderCollider extends CollisionShape {
	...
	public boolean intersection(SphereCollider sphereCollider) {
		...
		float dy = Math.max(this.tempCenter.y - height,
				Math.min(sphereCollider.getTempCenter().y, this.tempCenter.y + height));

		float distance = (float) Math.sqrt((this.tempCenter.x - sphereCollider.getTempCenter().x) *
				(this.tempCenter.x - sphereCollider.getTempCenter().x) +
				(dy - sphereCollider.getTempCenter().y) * (dy - sphereCollider.getTempCenter().y) +
				(this.tempCenter.z - sphereCollider.getTempCenter().z) *
						(this.tempCenter.z - sphereCollider.getTempCenter().z));

		if (distance < sphereCollider.getRadius() + this.radius) { isIntersecting = true; }
	...
	public boolean intersection(CylinderCollider cylinderCollider) {
		...
		if (Math.abs(this.tempCenter.x - cylinderCollider.getTempCenter().x) <
				(this.radius + cylinderCollider.getRadius()) &&
				(Math.abs(this.tempCenter.y - cylinderCollider.getTempCenter().y) <
						(this.height + cylinderCollider.getHeight()) &&
						(Math.abs(this.tempCenter.z - cylinderCollider.getTempCenter().z) <
								(this.radius + cylinderCollider.getRadius())))) { isIntersecting = true; }
