...
public class AABB extends CollisionShape {
	...
	private float convertVertices(float vertices, float cornerMin, float cornerMax) {
		if (vertices == 1) { return cornerMax; } else if (vertices == - 1) { return cornerMin; } else if (vertices ==
				0.98f) {
	...
	public boolean intersection(SphereCollider sphereCollider) {
		...
		float distance =
				(float) Math.sqrt((dx - sphereCollider.getTempCenter().x) * (dx - sphereCollider.getTempCenter().x) +
		...
		if (distance < sphereCollider.getRadius()) { isIntersecting = true; }
	...
	public boolean intersection(CylinderCollider cylinderCollider) {
		...
		float distance = (float) Math
				.sqrt((dx - cylinderCollider.getTempCenter().x) * (dx - cylinderCollider.getTempCenter().x) +
						(dz - cylinderCollider.getTempCenter().z) * (dz - cylinderCollider.getTempCenter().z));

		if (distance < cylinderCollider.getRadius() &&
				(Math.abs(this.tempCenter.y - cylinderCollider.getTempCenter().y) <
						(this.height + cylinderCollider.getHeight()))) { isIntersecting = true; }
