...
public class CollisionMeshHandler extends RendererHandler {
	...
	public void processModel(Model model) {
		...
		List<Model> batch =
				collisionShapeListMap.get(model.getComponent(CollisionComponent.class).getBroadPhaseCollisionShape());
		if (batch != null) {
			batch.add(model);
		} else {
			List<Model> newBatch = new ArrayList<>();
			newBatch.add(model);
			collisionShapeListMap
					.put(model.getComponent(CollisionComponent.class).getBroadPhaseCollisionShape(), newBatch);
		}
	}
